...
def get_prompts(
    ...
    if prompt_type == PromptType.SIMPLE:
        ...
        n_prompts = min(
            len(positive_nouns),
            len(negative_nouns),
            len(negative_infinitives),
            len(positive_infinitives),
        )
        pos_prompts = [
            formatter.format(NOUN=positive_nouns[i], VRB=positive_infinitives[i])
            for i in range(n_prompts)
        ]
        neg_prompts = [
            formatter.format(NOUN=negative_nouns[i], VRB=negative_infinitives[i])
            for i in range(n_prompts)
        ]
        neutral_prompts = None
        pos_answers = prompt_config.get(
        ...
        pos_prompts = [
            formatter.format(ADJ=positive_adjectives[i], FEEL=positive_feelings[i])
            for i in range(n_prompts)
        ]
        neg_prompts = [
            formatter.format(ADJ=negative_adjectives[i], FEEL=negative_feelings[i])
            for i in range(n_prompts)
        ]
        neutral_prompts = None
        pos_answers = prompt_config.get(
...
class CleanCorruptedDataset(torch.utils.data.Dataset):
    ...
    def get_num_pad_tokens(self) -> Int[Tensor, "batch"]:
        ...
        return (self.clean_tokens == self.tokenizer.pad_token_id).sum(dim=1)
    ...
    def get_dataloader(self, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        token_answer_dataset = TensorDataset(
            self.corrupted_tokens, self.clean_tokens, self.answer_tokens
        )
    ...
    def run_with_cache(
        ...
        for idx, (corrupted_tokens, clean_tokens, answer_tokens) in enumerate(bar):
            ...
            with torch.inference_mode():
                ...
                bar.set_description("run_with_cache: corrupted forward pass")
                ...
                # clean forward pass
                bar.set_description("run_with_cache: clean forward pass")
                ...
                bar.set_description("run_with_cache: filling buffer")
                ...
                clean_logit_diffs[start_idx:end_idx] = (
                    get_logit_diff(
                        clean_logits,
                        answer_tokens,
                        per_prompt=True,
                ...
                corrupted_logit_diffs[start_idx:end_idx] = (
                    get_logit_diff(
                        corrupted_logits,
                        answer_tokens,
                        per_prompt=True,
                ...
                clean_prob_diffs[start_idx:end_idx] = (
                    get_prob_diff(
                        clean_logits,
                        answer_tokens,
                        per_prompt=True,
                ...
                corrupted_prob_diffs[start_idx:end_idx] = (
                    get_prob_diff(
                        corrupted_logits,
                        answer_tokens,
                        per_prompt=True,
        ...
        corrupted_cache = ActivationCache(
            {
                ...
                for k, v in corrupted_dict.items()
            },
            model=model,
        )
...
class CleanCorruptedCacheResults:
    ...
    def set_accuracy(self):
        ...
        self.corrupted_accuracy = get_accuracy_from_logit_diffs(
...
def get_pickle_dataset(
    model: HookedTransformer, prompt_type: PromptType, scaffold: ReviewScaffold
):
    return load_pickle(prompt_type.value + "_" + scaffold.value, model)
